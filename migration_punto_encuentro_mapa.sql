...
SET @sql := IF(
    (SELECT COUNT(*) FROM information_schema.COLUMNS
      WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'pedidos' AND COLUMN_NAME = 'latitud_encuentro') = 0,
    'ALTER TABLE pedidos ADD COLUMN latitud_encuentro DOUBLE NULL COMMENT ''Latitud del punto de encuentro del viaje a la zona (NO es la casa del cliente)''',
    'SELECT ''pedidos.latitud_encuentro ya existe'' AS aviso');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @sql := IF(
    (SELECT COUNT(*) FROM information_schema.COLUMNS
      WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'pedidos' AND COLUMN_NAME = 'longitud_encuentro') = 0,
    'ALTER TABLE pedidos ADD COLUMN longitud_encuentro DOUBLE NULL COMMENT ''Longitud del punto de encuentro del viaje a la zona (NO es la casa del cliente)''',
    'SELECT ''pedidos.longitud_encuentro ya existe'' AS aviso');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
