...
public interface IVentaRepository extends BaseRepository<Venta, Integer> {
    ...
    // "Devuelta" (venta cancelada tras entrega, ver PedidoServiceImpl.deletePedidoById) se excluye
    // de fecha/reportes de ingresos para que el dinero reportado no cuente algo que se regreso.
    @Query("SELECT v FROM Venta v WHERE v.fechaVenta BETWEEN :desde AND :hasta " +
           "AND v.estadoVenta <> 'Devuelta' ORDER BY v.fechaVenta DESC")
    ...
    @Query("SELECT SUM(v.totalVenta), SUM(v.gananciaTotal), COUNT(v) FROM Venta v " +
           "WHERE v.fechaVenta BETWEEN :desde AND :hasta AND v.estadoVenta <> 'Devuelta'")
    ...
    @Query("SELECT FUNCTION('DATE', v.fechaVenta), SUM(v.totalVenta), SUM(v.gananciaTotal), COUNT(v) " +
           "FROM Venta v WHERE v.fechaVenta BETWEEN :desde AND :hasta AND v.estadoVenta <> 'Devuelta' " +
